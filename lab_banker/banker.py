...
class Banker:
    ...
    def request_resources(self, customer_index, request):
        ...
        ### BEGIN ANSWER HERE ###
        work_copy = deepcopy(self.available)
        need_copy = deepcopy(self.need)
        allocation_copy = deepcopy(self.allocation)
        safe = self.check_safe(customer_index, request, work_copy, need_copy, allocation_copy)  # Change this line
    ...
    def check_safe(self, customer_index, request, work, need, allocation):
        ...
        safe = all(finish[i] for i in range(length))
